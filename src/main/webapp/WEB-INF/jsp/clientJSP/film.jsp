...
<main>
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-7">
                <h2>${film.name}</h2>
                <p><fmt:message key="label.stageDirector"/> <a
                        href="<c:url value="/controller?command=VIEW_PERSON&stageDirectorName=${film.stageDirector.name}"/>">${film.stageDirector.name}</a>
                </p>
                ...
                <p><fmt:message key="label.actors"/>
                    <c:forEach items="${film.actors}" var="actor">
                        <a href="<c:url value="/controller?command=VIEW_PERSON&actorName=${actor.name}"/>">${actor.name};</a>
                    </c:forEach>
                </p>
        ...
        <div class="row">
            <c:choose>
                <c:when test="${user != null && user.blocked == false}">
                    ...
                    <textarea class="form-control" name="text"
                              placeholder="<fmt:message key="label.textComment"/>" required></textarea>
            ...
            <span><fmt:formatDate value="${comment.creationDate}"
                                  type="BOTH"/> ${commentUsers[status.index].login}</span>
