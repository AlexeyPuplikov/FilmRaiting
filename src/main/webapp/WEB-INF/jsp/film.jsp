...
<div class="col-md-7">
<h2>${film.name}</h2>
<p><fmt:message key="label.stageDirector"/> <a href="<c:url value="/controller?command=VIEW_PERSON&personName=${film.stageDirector.name}"/>">${film.stageDirector.name}</a></p>
    ...
    <c:forEach items="${film.actors}" var="actor">
        <a href="<c:url value="/controller?command=VIEW_PERSON&personName=${actor.name}"/>">${actor.name};</a>
    </c:forEach>
    </p>
