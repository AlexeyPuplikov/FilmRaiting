...
public class OpenAddFilmPageCommand implements ActionCommand {
    ...
    public String execute(HttpServletRequest request) {
        ...
        if (request.getParameter("errorAdd") != null) {
            request.setAttribute(PARAM_ADD_MESSAGE, configurationManager.getProperty(ADD_ERROR));
        }
        if (request.getParameter("successfulAdd") != null) {
            request.setAttribute(PARAM_ADD_MESSAGE, configurationManager.getProperty(ADD_SUCCESSFUL));
        }
        if (request.getParameter("successfulAddParameters") != null) {
            request.setAttribute(PARAM_ADD_MESSAGE, configurationManager.getProperty(ADD_PARAMETER_SUCCESSFUL));
        }
        if (request.getParameter("successfulAddCover") != null) {
            request.setAttribute(PARAM_ADD_MESSAGE, configurationManager.getProperty(ADD_COVER_SUCCESSFUL));
        }
        if (request.getParameter("errorAddParameters") != null) {
            request.setAttribute(PARAM_ADD_MESSAGE, configurationManager.getProperty(ADD_PARAMETERS_ERROR));
        }
